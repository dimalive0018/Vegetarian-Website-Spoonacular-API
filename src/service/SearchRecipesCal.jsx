...
export const SearchRecipesCal = () => {
  ...
  const searchedRecipes = async (type) => {
    const api = await base.get(`complexSearch?apiKey=${import.meta.env.VITE_API_KEY}&diet=vegetarian&number=100000&maxCalories=${type}`).then(({data}) => data);
    try {
  ...
  return (
    <>
      <SeoHelmet title={'Brocòle - Search by Calories'} description={"Brocòle - Search by Calories - powered by Spoonacular API"} />
      ...
      >
        <h2 className='text-center font-bold text-lg md:text-2xl xl:text-3xl border-b-4 border-[#608E43]'>Search results for {params.type} calories</h2>
      </motion.div>
      </AnimatePresence>
      {loading === true && (
        <AnimatePresence mode='wait'>
