...
export const SearchRecipesIngr = () => {
  ...
  const searchedRecipes = async (type) => {
    const api = await axios.get(`https://api.spoonacular.com/recipes/complexSearch?apiKey=${import.meta.env.VITE_SPO_API_KEY}&query=${type}&diet=vegetarian&number=100000`).then(({data}) => data);
    try {
